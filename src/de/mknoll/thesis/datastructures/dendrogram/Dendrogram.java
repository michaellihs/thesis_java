 ...
 */
public abstract class Dendrogram<T extends TagCloudContainer & AttachableToNode> {
	 ...
	 * 
	 * @param Number of partitions to be created. Must be bigger than 0!
	 * @return Partitioning consisting of a set of dendrograms
	 * @throws IllegalArgumentException if number of partitions is less than 1
	 * @throws IllegalArgumentException if number of partitions is bigger than contained elements
	 */
	public Set<Dendrogram<T>> partitionDendrogram(Integer k) {
		if (k < 1) {
			throw new IllegalArgumentException("Number of partitions must be bigger or equal than 1!");
		} else if (k == 1) {
			// Return whole dendrogram if partitioning with only one partition should be made
	...
	/**
	 * Returns a set of sets of node objects generated by "cutting" dendrogram such that
	 * k sub-dendrograms are generated.
	 * 
	 * @param Number of partitions to be created. Must be bigger than 0!
	 * @return Partitioning consisting of a set of set of nodes
	 */
	public HashSet<HashSet<Node>> nodePartitioning(Integer k) {
		Set<Dendrogram<T>> dendrogramSet = this.partitionDendrogram(k);
		HashSet<HashSet<Node>> partitioningAtK = new HashSet<HashSet<Node>>();
		for (Dendrogram<T> d : dendrogramSet) {
			HashSet<Node> nodeSet = new HashSet<Node>();
			for (T containedObject : d.memberSet()) {
				nodeSet.add(containedObject.getNode());
			}
			partitioningAtK.add(nodeSet);
		}
		return partitioningAtK;
	}
	
	
	
	/**
	 * Returns tag cloud of given dendrogram node
	 * 
	 * @return Tag cloud of given dendrogram node
	 */
